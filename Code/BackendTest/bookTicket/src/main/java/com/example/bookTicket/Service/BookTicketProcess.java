			...
			if (trainSeatClass.isPresent() && seatCounts.size() == 1) {
				...
				seatCounts.forEach(data->{
					if (data.getSeatRemainingCount() > bookTicketRequest.getReservedSeatCount()) {
						List<SeatInfo> createdSeatInfos = new ArrayList<SeatInfo>();
						for (int i = 0; i < bookTicketRequest.getReservedSeatCount(); i++) {
							...
							//save seat and keep reference for TicketSeats creation
							createdSeatInfos.add(seatInfoRepo.save(newSeat));
						}
						...
						// choose return station (optional)
						Stations returnStation = stations.isEmpty() ? null : stations.get(0);
						TrainTime tt = trainTimeRepo.findById(bookTicketRequest.getTrainTimeID()).orElseThrow();
						// persist main ticket
						BookedTickets savedTicket = bookedTicketRepo.save(
								new BookedTickets(
						...
						createdSeatInfos.forEach(seat -> {
							com.example.bookTicket.Entity.TicketSeats ts = new com.example.bookTicket.Entity.TicketSeats(savedTicket, seat);
							ticketSeatsRepo.save(ts);
						});
					}else {
						//return "no seats"
					}
