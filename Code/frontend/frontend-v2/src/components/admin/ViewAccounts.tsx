...
import { Eye, ArrowRight, Ticket, Activity } from 'lucide-react';

const MOCK_USERS = [
  { id: 'USR-892', name: 'Alice Johnson', email: 'alice@example.com', tickets: 12, joined: '2023-05-12' },
  { id: 'USR-893', name: 'Bob Smith', email: 'bob@example.com', tickets: 3, joined: '2023-08-21' },
  { id: 'USR-894', name: 'Charlie Davis', email: 'charlie@example.com', tickets: 0, joined: '2023-11-05' },
];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <div className="flex items-center gap-6">
          <div className="h-20 w-20 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-display text-3xl font-bold border-2 border-indigo-200">
            {user.name.charAt(0)}
          </div>
          <div>
            <h2 className="text-2xl font-bold text-slate-900">{user.name}</h2>
            <p className="text-slate-500">{user.email} • ID: <span className="font-mono">{user.id}</span></p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
            <Ticket className="h-5 w-5 text-indigo-600" /> Tickets Booked ({user.tickets})
          </h3>
          {user.tickets > 0 ? (
              ...
              {[1, 2].map(i => (
                <div key={i} className="p-4 border border-slate-100 rounded-xl bg-slate-50/50">
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-mono text-xs text-slate-500">#TCK-992{i}</span>
                    <span className="text-xs font-medium text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded-full">Completed</span>
                  </div>
                  <p className="font-medium text-slate-900">Express InterCity 402</p>
                  <p className="text-sm text-slate-600 mt-1">NYP → BOS • Oct 12, 2023</p>
                </div>
              ))}
            </div>
          ) : (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="relative border-l border-slate-200 ml-3 space-y-6 pb-4">
            {[
              ...
              { action: 'Viewed schedule for TRN-002', time: 'Yesterday, 14:20 PM' },
              { action: 'Booked ticket #TCK-9922', time: 'Oct 12, 2023, 10:05 AM' },
