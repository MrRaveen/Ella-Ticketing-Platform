...
export default function PassengerLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save the JWT token to localStorage (check multiple possible field names)
      const token = data.jwtToken || data.token || data.jwt || data.accessToken;
      if (token) {
        localStorage.setItem('token', token);
        console.log('Token saved:', token);
      } else {
        console.warn('No token found in response. Keys:', Object.keys(data));
      }
